...
RWStructuredBuffer<uint64_t> gSortKeys : BAMBOO_PASS_UAV(LIGHT_TREE_REG_SORT_KEYS); // SIByL u_Codes
...
RWStructuredBuffer<uint> gCompactIds : BAMBOO_PASS_UAV(LIGHT_TREE_REG_COMPACT_IDS); // SIByL u_compactIndex (compactID -> voxelID)
...
RWStructuredBuffer<float> gSpixelClusterImportanceHeap : BAMBOO_PASS_UAV(LIGHT_TREE_REG_IMPORTANCE_HEAP); // SIByL tltree (spixelFlat*64 implicit heap)
RWTexture2D<uint> gClusterVisibilityMask : BAMBOO_PASS_UAV(LIGHT_TREE_REG_VISIBILITY_MASK);              // SIByL visibilityIMG (bit c = sees cluster c)
...
// ---- BuildSuperpixelClusterHeaps (tree-top-level-constr) --------------------
// Top-level tree. One warp (32 lanes) per superpixel, lane = cluster index. For
// each cluster it reads that cluster's root-node intensity (0 if the cluster has
// no root), gates it by the superpixel's view visibility, then folds the 32
// values into an implicit binary heap: leaves [32..63], pairwise sums up to
